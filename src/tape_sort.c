...
void tape_sort_insert(address_3bc_t addres)
{
    ...
    if (new_tape == NULL) {
        driver_program_error(ERROR_TAPE_SORT);
    }
    ...
    while (index >= 1) {
        ...
        data_3bc_t prev_value = driver_memory_data_get(prev_addres);
        address_3bc_t addres = addresses_list[index];
        data_3bc_t value = driver_memory_data_get(addres);
        ...
        /** spin values **/
        driver_memory_data_set(prev_addres, value);
        driver_memory_data_set(addres, prev_value);
