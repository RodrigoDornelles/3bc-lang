...
{
    VALIDATE_NOT_DUALITY
    tape_aux_set((data_3bc_t) pow((double) (AUX), (double) (GET_ANY_PARAM)));
}

void cpu_math_root(PARAMS_DEFINE)
{
    VALIDATE_NOT_DUALITY
    tape_aux_set((data_3bc_t) pow((double) (AUX), (1 / (double) (GET_ANY_PARAM))));
}

void cpu_math_percentage(PARAMS_DEFINE)
{
    VALIDATE_NOT_DUALITY
    tape_aux_set((data_3bc_t) (AUX) * (((double) GET_ANY_PARAM)/100.0));
}

void cpu_math_abs(PARAMS_DEFINE)
{
    VALIDATE_NOT_DUALITY
    tape_aux_set((data_3bc_t) labs((long int) AUX));
}