...
class TestFails < Minitest::Test
    def test_cpu_zero
        stdout, stderr, status = Open3.capture3('./bin/3bc-debug', :stdin_data => "strc 0 0")
    ...
    def test_cpu_reserved
        ...
        stdout, stderr, status = Open3.capture3('./bin/3bc-debug', :stdin_data => "mode 0 #{cpu_mode}\n1 0 0")
    ...
    def test_invalid_address
        stdout, stderr, status = Open3.capture3('./bin/3bc-debug', :stdin_data => "mode e nill")
        assert_match /ERROR CODE\:(\t| )?(0x3BC003)/, stderr
        assert_equal 15, status.exitstatus
    end

    def test_invalid_constant
        stdout, stderr, status = Open3.capture3('./bin/3bc-debug', :stdin_data => "mode nill e")
        assert_match /ERROR CODE\:(\t| )?(0x3BC004)/, stderr
        assert_equal 15, status.exitstatus
    end

    def test_invalid_cpu_mode
        stdout, stderr, status = Open3.capture3('./bin/3bc-debug', :stdin_data => "mode nill full")
        assert_match /ERROR CODE\:(\t| )?(0x3BC005)/, stderr
        assert_equal 15, status.exitstatus
    end

    def test_invalid_label
        stdout, stderr, status = Open3.capture3('./bin/3bc-debug', :stdin_data => "nill nill 1\nnill nill 1")
        assert_match /ERROR CODE\:(\t| )?(0x3BC006)/, stderr
        assert_equal 15, status.exitstatus
    end

    def test_invalid_return
        stdout, stderr, status = Open3.capture3('./bin/3bc-debug', :stdin_data => "mode.0.41,back.0.0")
    ...
    def test_param_required_any
        stdout, stderr, status = Open3.capture3('./bin/3bc-debug', :stdin_data => "mode.0.43,fake.0.0")
        assert_match /ERROR CODE\:(\t| )?(0x3BC009)/, stderr
        assert_equal 15, status.exitstatus
    end

    def test_param_required_value
        stdout, stderr, status = Open3.capture3('./bin/3bc-debug', :stdin_data => "mode 0 9\ngoto 0 0")
        assert_match /ERROR CODE\:(\t| )?(0x3BC00A)/, stderr
        assert_equal 15, status.exitstatus
    end

    def test_param_required_address
        stdout, stderr, status = Open3.capture3('./bin/3bc-debug', :stdin_data => "mode 0 6\nfree 0 0")
    ...
    def test_number_no_digits
        stdout, stderr, status = Open3.capture3('./bin/3bc-debug', :stdin_data => "mode 0 --")
        assert_match /ERROR CODE\:(\t| )?(0x3BC00E)/, stderr
        assert_equal 15, status.exitstatus
    end

    def test_number_underflow
        stdout, stderr, status = Open3.capture3('./bin/3bc-debug', :stdin_data => "mode.0.2,strx.0.-0xFFFFFFFFFFFFFFFF")
    ...
    def test_no_such_file
        stdout, stderr, status = Open3.capture3('./bin/3bc-debug', "none.3bc")
    ...
    def test_invalid_char_scape
        stdout, stderr, status = Open3.capture3('./bin/3bc-debug', :stdin_data => "0.0.'\\N'")
