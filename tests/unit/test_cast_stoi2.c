...
{
    {
        tbc_u8_t dest = 25u;
        char* src = "01100110";
        assert(cast_stoi2(&dest, src, 8, 8) == ERROR_UNKWON);
        assert(dest == 0x66);
    }
    {
        tbc_u16_t dest = 25042u;
        char* src = "0110011001100110";
        assert(cast_stoi2(&dest, src, 16, 16) == ERROR_UNKWON);
        assert(dest == 0x6666);
    }
    {
        tbc_u32_t dest = 20230425lu;
        char* src = "11001100110011001100110011001100";
        assert(cast_stoi2(&dest, src, 32, 32) == ERROR_UNKWON);
    ...
    }
    {
        ...
        assert(cast_stoi2(&dest, src, 64, 64) == ERROR_UNKWON);
