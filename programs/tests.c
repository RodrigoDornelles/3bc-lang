...
{
    driver_program_error(app, (tbc_error_et)value);
}
...
>>>>>>> afd87efc06b0669489ec4f6696fbb61906f3fed6
}

int main(int argc, char** argv)
{
    struct app_3bc_s* const VM = lang_3bc_init(argc, argv);
    lang_3bc_custom(VM, TBC_MODE_CUSTOM_1, 1, &cpu_test_signal);
    lang_3bc_custom(VM, TBC_MODE_CUSTOM_1, 2, &cpu_test_none_tty);
    lang_3bc_custom(VM, TBC_MODE_CUSTOM_1, 3, &cpu_test_unsupported);
    lang_3bc_custom(VM, TBC_MODE_CUSTOM_1, 4, &cpu_test_scape_characters);
    lang_3bc_custom(VM, TBC_MODE_CUSTOM_1, 5, &cpu_test_error);
